#pragma once
namespace utils {
	void initData(std::vector<Enemy> &enemies, std::map<std::string, weapons> &weaponReg) {
...
#pragma endregion
		...
		int weaponCounter = 0;
		//Weapon nodes
		for (rapidxml::xml_node<> *pNode = pRoot->first_node("weapon"); pNode; pNode = pNode->next_sibling("weapon")) {
			if (weaponReg.count(pNode->first_node("id")->value()) != 0) {
				...
				weaponReg.insert({ newWeapon.identifier, newWeapon });
			}
			weaponCounter++;
		}
		...
		std::string name;
		int enemyWeaponCounter = 0;
		pRoot = doc.first_node();

		for (rapidxml::xml_node<> *pNode = pRoot->first_node("enemy"); pNode; pNode = pNode->next_sibling("enemy")) {
			weaponsList.clear();
